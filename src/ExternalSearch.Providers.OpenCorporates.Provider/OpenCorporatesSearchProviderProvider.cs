...
using CluedIn.ExternalSearch.Providers.OpenCorporates;
using CluedIn.ExternalSearch.Providers.OpenCorporates.Vocabularies;
using CluedIn.Providers.Models;
using Constants = CluedIn.ExternalSearch.Providers.OpenCorporates.Constants;

namespace CluedIn.Provider.OpenCorporates
{
    public class OpenCorporatesSearchProviderProvider : ProviderBase, IExtendedProviderMetadata, IExternalSearchProviderProvider
    {
        public IExternalSearchProvider ExternalSearchProvider { get; }

        public OpenCorporatesSearchProviderProvider([System.Diagnostics.CodeAnalysis.NotNull] ApplicationContext appContext) : base(appContext, GetMetaData())
        {
        ...
        public override async Task<CrawlJobData> GetCrawlJobData(ProviderUpdateContext context, IDictionary<string, object> configuration, Guid organizationId, Guid userId, Guid providerDefinitionId)
        {
            ...
            var result = new OpenCorporatesExternalSearchJobData(configuration);
        ...
        {
            if (jobData is OpenCorporatesExternalSearchJobData result)
            {
                return await Task.FromResult(result.ToDictionary());
            }

            throw new InvalidOperationException($"Unexpected data type for {nameof(OpenCorporatesExternalSearchJobData)}, {jobData.GetType()}");
        }
        ...
        public Dictionary<string, object> ExtraInfo { get; } = new Dictionary<string, object>
        {
            ...
            { "nameKeyField", Constants.KeyName.LookupVocabularyKey },
            { "vocabKeyPrefix", OpenCorporatesVocabulary.Organization.KeyPrefix },
